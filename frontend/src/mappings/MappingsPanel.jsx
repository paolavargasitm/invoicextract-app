...
export default function MappingsPanel({ theme }) {
  const [erp, setErp] = useState('SAP');
  const [status, setStatus] = useState('ACTIVE');
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [known, setKnown] = useState({ invoiceFields: [], itemFields: [], tables: {} });
  ...
  useEffect(() => { if (erp) load(); /* eslint-disable-next-line */ }, [erp, status]);

  // Load reference fields once
  useEffect(() => {
  ...
  const fieldMeta = (name) => {
    ...
    const search = (tbl) => (tbl?.fields || []).find(f => f.name === name);
  ...
  const updateRow = async (row) => {
    setError('');
    try {
      await mappingsApi.update(row.id, {
        sourceField: row.sourceField,
        targetField: row.targetField,
        transformFn: row.transformFn,
        status: row.status,
      });
      await load();
    } catch (e) { setError(e.message); }
  ...
  const toggleStatus = async (row) => {
    ...
    try { await mappingsApi.changeStatus(row.id, next); await load(); } catch (e) { setError(e.message); }
  };

  const StatusBadge = ({ s }) => s === 'ACTIVE' ? badge('Activa', '#16a34a') : badge('Inactiva', '#ef4444');
    ...
    <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 12, padding: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <input value={erp} onChange={e => setErp(e.target.value)} placeholder="ERP (p.ej. SAP)" style={{ padding: 8, borderRadius: 8, border: `1px solid ${theme.border}` }} />
          <select value={status} onChange={e => setStatus(e.target.value)} style={{ padding: 8, borderRadius: 8, border: `1px solid ${theme.border}` }}>
      ...
      <div style={{ marginTop: 16, borderTop: `1px solid ${theme.border}`, paddingTop: 12 }}>
        ...
        <form onSubmit={submit} style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          <input value={form.erpName} onChange={(e) => setForm({ ...form, erpName: e.target.value })} placeholder="ERP" style={{ padding: 8, borderRadius: 8, border: `1px solid ${theme.border}` }} />
